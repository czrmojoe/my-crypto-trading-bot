...
def monitor_crypto_price():
    # เปลี่ยนการเชื่อมต่อ CCXT ให้ดึงข้อมูลจากตลาดเหรียญฟิวเจอร์ส (Binance Futures)
    exchange = ccxt.binance({
        'enableRateLimit': True,
        'options': {
            'defaultType': 'future', # 👈 บังคับดึงข้อมูลจากกระดาน Futures
        }
    })
    
    # ตั้งราคาแจ้งเตือนแนวรับ/แนวต้านฝั่งฟิวเจอร์ส
    target_high = 85000  
    target_low = 65000
    ...
    while True:
        try:
            # ดึงราคาคู่สัญญา Perpetual (BTC/USDT)
...
def send_welcome(message):
    ...
    bot.reply_to(message, "สวัสดีครับน้า! บอทเวอร์ชันอัปเกรดราคาสดกระดาน Futures Perpetual พร้อมลุย 🤖⚡\n\n"
...
# ฟังก์ชันพิมพ์ข้อความคุยโต้ตอบ + ดักจับเช็คราคาด่วนฝั่ง Futures
...
def handle_text_chat(message):
    ...
    if any(keyword in msg_text for keyword in price_keywords):
        try:
            ...
            # ดึงราคาตรงจากกระดานตลาดล่วงหน้า
