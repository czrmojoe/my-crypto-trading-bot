...
def monitor_crypto_price():
    # ใช้ตลาด fapi (Binance Futures API) โดยตรงเพื่อความเสถียร
    ...
    while True:
        try:
            # ดึงราคาจากกระดาน Futures ใช้ชื่อคู่สัญญาไร้สแลช 'BTCUSDT'
...
def handle_text_chat(message):
    ...
    if any(keyword in msg_text for keyword in price_keywords):
        try:
            ...
            # ดึงข้อมูลจากกระดานฟิวเจอร์สของ Binance 
            exchange = ccxt.binance({'options': {'defaultType': 'future'}})
            
            # ใช้คู่ 'BTCUSDT' และดึงผ่านฟังก์ชันหลัก ล็อกราคาแมตช์จริงล่าสุดเพื่อความแม่นยำ
            ticker = exchange.fetch_ticker('BTCUSDT')
